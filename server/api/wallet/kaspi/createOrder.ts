...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  console.log('[api/wallet/kaspi/createOrder.ts] ⚠️⚠️⚠️ ЗАПРОС ПОЛУЧЕН:', {
    method: req.method,
    url: req.url,
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      authMiddleware(expressReq, expressRes, (err?: any) => {
        if (err) {
          console.error('[api/wallet/kaspi/createOrder.ts] Auth error:', err);
          return reject(err);
        }
        if (expressRes.headersSent) {
          console.log('[api/wallet/kaspi/createOrder.ts] Response already sent by auth');
          return;
        }
        resolve();
      });
    ...
    await new Promise<void>((resolve, reject) => {
      validateBody(addFundsSchema)(expressReq, expressRes, (err?: any) => {
        if (err) {
          console.error('[api/wallet/kaspi/createOrder.ts] Validation error:', err);
          return reject(err);
        }
        if (expressRes.headersSent) {
          console.log('[api/wallet/kaspi/createOrder.ts] Response already sent by validation');
          return;
        }
        resolve();
      });
    ...
    // Вызываем контроллер
    console.log('[api/wallet/kaspi/createOrder.ts] Вызываем createKaspiDepositOrder');
    await createKaspiDepositOrder(expressReq, expressRes);
  } catch (error: any) {
    console.error('[api/wallet/kaspi/createOrder.ts] Ошибка:', error);
